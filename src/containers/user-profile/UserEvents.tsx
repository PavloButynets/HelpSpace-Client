...
const MOCK_EVENTS = {
  created: [
    ...
    },
    {
      ...
      description: "Collect and distribute winter clothing to those in need before the cold season begins.",
      status: "UPCOMING",
      city: "Kyiv",
  ...
  participated: [
    {
      ...
      description: "Join us in cleaning up the riverbanks and removing plastic waste from our city's river.",
      status: "COMPLETED",
      city: "Lviv",
  ...
  return (
    <Box sx={{ p: 2 }}>
      <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
        <Tabs aria-label="events tabs" onChange={handleTabChange} value={tabValue} variant="fullWidth">
          <Tab label="Created Events" />
          <Tab label="Participated Events" />
        </Tabs>
      </Box>
      ...
      <TabPanel index={0} value={tabValue}>
        <Box sx={{ display: "flex", flexDirection: "column", gap: 2, mt: 1 }}>
          {events.created.length === 0 ? (
            ...
            events.created.map((event) => <EventCard event={event} key={event.id} showCreator={false} />)
          )}
        </Box>
      </TabPanel>
      ...
      <TabPanel index={1} value={tabValue}>
        <Box sx={{ display: "flex", flexDirection: "column", gap: 2, mt: 1 }}>
          {events.participated.length === 0 ? (
            ...
            events.participated.map((event) => <EventCard event={event} key={event.id} showCreator />)
          )}
        </Box>
      </TabPanel>
    </Box>
    ...
    <Card variant="outlined">
      <CardContent>
        ...
        >
          ...
          <Chip color={getStatusChipColor(event.status) as any} label={event.status} size="small" />
        </Box>
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 1, mb: 2 }}>
          <Box sx={{ display: "flex", alignItems: "center" }}>
            ...
            <Typography variant="body2">{formatEventDate(event.startDate, event.endDate)}</Typography>
          </Box>
