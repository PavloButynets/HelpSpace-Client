...
const MOCK_IMPACT_DATA = {
  categories: [
    { name: 'Навколишнє середовище', hours: 62, events: 8 },
    { name: 'Соціальні події', hours: 45, events: 7 },
    { name: 'Навчання', hours: 28, events: 5 },
    { name: "Здоров'я", hours: 21, events: 3 }
  ],
  monthly: [
    { name: 'Jan', hours: 12 },
    { name: 'Feb', hours: 8 },
...
export default function UserStats({ userId }: UserStatsProps) {
  ...
  const totalHours = stats.categories.reduce(
    ...
    <Box sx={{ p: 2 }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, mt: 2 }}>
        <Card variant='outlined'>
          <CardHeader
            subheader='Розподіл годин за категоріями'
            subheaderTypographyProps={{ variant: 'body2' }}
            title='Статистика за категоріями'
                  ...
                  >
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      ...
                      <Typography
                        ...
                        variant='body2'
                      >
                        ({category.events} подій)
                      </Typography>
                    </Box>
                    <Typography variant='body2'>
                      {category.hours} годин
                    </Typography>
                  </Box>
                  <LinearProgress
        ...
        <Card variant='outlined'>
          <CardHeader
            subheader='Розподіло годин по місяцях'
            subheaderTypographyProps={{ variant: 'body2' }}
            title='Місячна статистика'
            titleTypographyProps={{ variant: 'h6' }}
          />
          <CardContent>
            <Box sx={{ height: 300, width: '100%' }}>
              <BarChart
                height={300}
                series={[{ data: chartData, label: 'Години' }]}
